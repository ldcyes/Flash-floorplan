...
assert len(cell_area)>50, 'Liberty parser failed'
...
plan={'suite_version':1,'mode':MODE,'families':['crossbar','benes','router'],'configs':CONFIGS,'calibration':[[4,8],[8,8]],'routing_budgets':BUDGETS,'threads':1,'seed':42,'clock_period_ns':5,'timing_scope':'pre-CTS; no signoff timing or power','area_scope':'mapped standard-cell area, not die area','hotspot_threshold':.8,'reference':'Nangate45 typical; not foundry silicon'}
(OUT/'protocol.json').write_text(json.dumps(plan,indent=2))

def run(cmd,cwd,log,timeout=240,env=None):
...
# All 4-port permutations must be expressible by the 6 Beneš controls.
...
for kind in plan['families']:
 for n,w in CONFIGS:
    ...
    rec={'case':name,'family':kind,'n':n,'w':w,'split':'calibration' if (n,w) in [(4,8),(8,8)] else 'test','routes':[],'status':'started'}
    rows.append(rec)
    try:
...
'''
        ...
        rec['synthesis_s']=run(['yosys','-s','synth.ys'],d,'synthesis.log',timeout=300)
...
'''
        ...
        rec['placement_s']=run(['openroad','-exit','place.tcl'],d,'placement.log',timeout=300)
...
set t0 [clock milliseconds]
global_route -guide_file {budget}.guide -congestion_iterations 50 -allow_congestion -congestion_report_file {budget}.congestion.rpt
puts "GLOBAL_ROUTE_MS [expr {{[clock milliseconds]-$t0}}]"
write_db {budget}.odb
'''
            (d/f'{budget}.tcl').write_text(route)
            try:
                rr['openroad_wall_s']=run(['openroad','-exit',f'{budget}.tcl'],d,f'{budget}.log',timeout=300)
                txt=(d/f'{budget}.log').read_text(); rr['global_route_s']=float(re.search(r'GLOBAL_ROUTE_MS (\d+)',txt).group(1))/1000
                env={**os.environ,'INPUT_DB':str(d/f'{budget}.odb'),'OUTPUT_JSON':str(d/f'{budget}.physical.json')}
                rr['extraction_s']=run(['openroad','-python','-exit',str(ROOT/'benchmarks/extract.py')],d,f'{budget}.extract.log',env=env)
                phy=json.loads((d/f'{budget}.physical.json').read_text())
                rec['placed_area_um2']=phy['cell_area_um2'];rr['grid']=[len(phy.get('grid_x',[])),len(phy.get('grid_y',[]))]
                assert phy.get('layers') and any(sum(sum(r) for r in l['usage'])>0 for l in phy['layers']), 'No routed usage extracted'
                rr['status']='ok'
